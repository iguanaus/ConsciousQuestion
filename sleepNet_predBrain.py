...
def packageForRNN(data,seq_length,batch_size,num_epochs):
	...
	def gen_epochs(n,numsteps, batch_size):
		for i in xrange(n):
			yield ptb_iterator_old(data,batch_size,numsteps)
...
def file_data(filename="snippet-25_D_downsampled_20_numpy.npy"):
	...
	#Gonna do it here cause why not.
	seq_length = 50
	batch_length = 20

	#Now repackage it and all that shi
	#Now only make it one dimensional to test it. 
	print(retVal)
	...
	with tf.Session(config=tf.ConfigProto(log_device_placement=False, allow_soft_placement=False)) as sess:
		...
		for epoch in data:
			...
			for stepa, (X,Y) in enumerate(epoch):
				...
				# Batch feeding rnn:
				# feed in an entire sequence length.
				# Each batch is 
				#
